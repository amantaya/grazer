...
The area covered by a sampling frame is not recorded in the data, so we need to specify the dimensions of the sampling frame to calculate the area covered by the sampling frame. In this example, we will assume that the sampling frame is 50 cm x 50 cm (0.25 m^2).
...
We will calculate the forage production per unit area (kg/ha) for each pasture by first averaging the sample weights across all sampling locations within each pasture.
...
Finally, we can calculate the forage production per unit area (kg/ha) for each pasture using the `calc_forage_prod` function. This function scales the average sample weight to the desired unit (e.g., kg/ha, lb/acre) based on the area covered by the sampling frame.
