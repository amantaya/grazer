...
And load the example forage data.
...
Each row within the example data represents a different pasture, and the columns (e.g., Sample_1, Sample_2) represents the weight of clipped and dried forage (in grams) within the sampling frame at multiple sampling sites within the same pasture.
...
We will calculate the forage production per unit area (kg/ha) for each pasture by averaging the forage production across all sampling locations within each pasture and scaling it to the desired unit.

First, we need to calculate an average sample weight from all sampling locations within a pasture.
...
Finally, we can calculate the forage production per unit area (kg/ha) for each pasture using the `calc_forage_prod` function from the `grazer` package. This function scales the average sample weight to the desired unit (e.g., kg/ha, lb/acre) based on the area covered by the sampling frame.
